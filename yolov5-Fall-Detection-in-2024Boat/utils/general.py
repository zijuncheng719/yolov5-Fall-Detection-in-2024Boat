...
def check_dataset(data, autodownload=True, use_polyaxon=False):
    ...
    if use_polyaxon:
        ...
        data['train'] = rsync_data.get_polyaxon_dataroot(data_node="ceph", dir=data['train'])
        data['val'] = rsync_data.get_polyaxon_dataroot(data_node="ceph", dir=data['val'])
    # Parse yaml
    ...
    class_name, class_dict = parser_classes(data['names'], use_polyaxon=use_polyaxon)
...
def parser_classes(class_name, use_polyaxon=False):
    """
    ...
    if isinstance(class_name, str):
        if use_polyaxon:
            from utils import rsync_data
            class_name = rsync_data.get_polyaxon_dataroot(data_node="ceph", dir=class_name)
        class_name = read_files(class_name)
